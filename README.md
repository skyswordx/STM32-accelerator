...
在 adc_processing_task 任务中的循环实现更精细的控制
- 先采样一个缓存区（16k）的ADCdata
- 采集满了之后就停止采集
- 接着输出所有缓存区中的 ADCdata 作为时域波形
- 延时3秒钟
- 对这个缓存区分 4 段，先对第一段进行FFT处理，计算每段的幅度谱
- 然后同时用printf()输出这一段时域波形、幅度谱（只做ADC1的先）

可能需要修改一下相关的接口，比如 ProcessCompleteBuffer 以及 buffer_fill_count++ 等，实现更加灵活的 ADC 采样和处理。