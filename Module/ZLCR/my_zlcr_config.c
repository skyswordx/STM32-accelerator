...
void set_dds_frequency(double frequency) {
    // 这里可以根据系统中实际使用的DDS芯片来实现
    // 当前系统中似乎同时使用了AD9833和AD9954，需要根据实际情况选择
    ...
    // 在实际应用中，需要根据硬件连接和配置选择合适的DDS芯片
    printf("Setting DDS frequency to %.2f Hz\n", frequency);
}
...
void set_dds_mode(uint16_t mode) {
    ...
    // 示例：使用AD9833设置模式
    // AD9833可以设置正弦波、三角波、方波等模式
    // switch (mode) {
    //     case 0: // 正弦波
    //         AD9833_WaveSeting(1000.0, 0, SIN_WAVE, 0); // 频率1kHz，正弦波
    //         break;
    //     case 1: // 三角波
    //         AD9833_WaveSeting(1000.0, 0, TRI_WAVE, 0); // 频率1kHz，三角波
    //         break;
    //     case 2: // 方波
    //         AD9833_WaveSeting(1000.0, 0, SQU_WAVE, 0); // 频率1kHz，方波
    //         break;
    //     default:
    //         AD9833_WaveSeting(1000.0, 0, SIN_WAVE, 0); // 默认正弦波
    //         break;
    // }
    
    // 示例：使用AD9954设置模式
    // AD9954可以设置更复杂的模式，如FSK、PSK等
    
    // 在实际应用中，需要根据硬件连接和配置选择合适的DDS芯片
    printf("Setting DDS mode to %d\n", mode);
}
