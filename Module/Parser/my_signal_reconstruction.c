...
static int32_t analyze_generic_fft(float32_t* adc_input, harmonic_component_t* components, int32_t max_components) {
    ...
    for(int n = FFT_LENGTH - 1; n >= 0; n--) { 
        g_fft_input_buffer[2*n] = g_fft_input_buffer[n]; 
        g_fft_input_buffer[2*n+1] = 0.0f; 
    }
    ...
    fund_idx++; // arm_max_f32 返回的是子数组内的索引
    const float32_t peak_threshold = fund_mag * 0.01f; // 噪声阈值：基波的1%
...
static wave_type_t guess_wave_type(const harmonic_component_t* measured, int32_t num_measured) {
    ...
    if (even_harmonic_energy / measured[0].amplitude < 0.05f) {
        // 在此可以进一步通过A3/A1的比例区分方波和三角波，此处简化为猜测方波
        return WAVE_SQUARE;
    }
...
analysis_method_t analyze_and_select_best_method(
    ...
    if (enable_math_synthesis) {
        ...
        // 2b. 如果猜测是标准波形，则采用数学合成法
        if (guessed_type == WAVE_SQUARE || guessed_type == WAVE_TRIANGLE || guessed_type == WAVE_SINE) {
            printf("Decision: Signal appears to be a standard wave. Using ideal harmonics.\r\n");
            // 使用测量的基波参数和猜测的类型来生成理想谐波
...
void reconstruct_output_waveform(
    ...
    for (int i = 0; i < num_components; i++) {
        ...
        // --- 核心优化：利用100Hz步长特性进行直接地址计算 ---
        // 通过公式直接计算出当前谐波频率在扫频数据中的索引，避免搜索和插值
